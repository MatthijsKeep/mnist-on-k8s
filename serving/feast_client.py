...
REPO_YAML = os.environ.get('FEAST_REPO_CONFIG', 'features/feast_repo/feature_store.yaml')
...
def get_online_features(image_id: int) -> np.ndarray:
    ...
    redis_password = os.environ.get('REDIS_PASSWORD', '')  # Empty fallback (no hardcode; use env only)

    config.online_store.type = "redis"
    
    # Build connection_string with password if provided
    if redis_password:
        config.online_store.connection_string = f"{redis_host}:6379,password={redis_password}"
    else:
