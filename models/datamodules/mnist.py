...
class MNISTDataset(Dataset):
    def __init__(self, image_data: np.ndarray, stats_data: np.ndarray, labels_data: np.ndarray, is_train: bool = False, stats_scaler=None):
        # Convert to tensors once for efficiency + normalization
        images = torch.tensor(image_data, dtype=torch.float32).unsqueeze(1).view(-1, 1, 28, 28)  # (N, 1, 28, 28)
        ...
        assert self.image.shape[1] == 1 and self.image.shape[2] == self.image.shape[3] == 28, f"Image shape mismatch: {self.image.shape}"
