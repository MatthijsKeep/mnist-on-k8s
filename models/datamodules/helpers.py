...
def fetch_and_prepare_features(repo_path="features/feast_repo", n_samples=60000):
    ...
    entity_df = pd.DataFrame(
        {"image_id": range(n_samples), "event_timestamp": [datetime.now()] * n_samples}
    )
    ...
    # Prepare arrays
    assert np.frombuffer(training_df["flat"].iloc[0], dtype=np.float32).shape == (
        784,
    ), "Flat image shape mismatch, must be (784,)"
    training_df["flat"] = training_df["flat"].apply(
        lambda x: np.array(
    ...
    print(
        f"Train sizes: image {train_image.shape}, stats {train_stats.shape}, y {train_y.shape}"
    )
    print(
        f"Val sizes: image {val_image.shape}, stats {val_stats.shape}, y {val_y.shape}"
    )
